...
class idiffnet():
    ...
    # user_group_embeddings用户记录用户的组别 [num_user, num_group]
    def getAHatGroup(self, snsm, user_group_embeddings):
        group_embedding = tf.transpose(user_group_embeddings)
        A_fold_hat_group = []
        A_fold_hat_group_filter = []
        for k in range(self.conf.group):
            A_fold_item_filter = []
            A_fold_hat_item = []
            for fold in range(1):
                A_fold_hat_item.append(snsm.__mul__(group_embedding[k]) \
                                       .__mul__(tf.transpose(group_embedding[k])))
                item_filter = tf.sparse_reduce_sum(A_fold_hat_item[0], axis=1)
    ...
    def initializeNodes(self):
        ...
        self.w_gc_1 = tf.Variable(self.initializer([self.conf.dimension, self.conf.dimension]), name="w_gc_1")
        self.b_gc_1 = tf.Variable(self.initializer([1, self.conf.dimension]), name="b_gc_1")

        self.w_gc = tf.Variable(self.initializer([self.conf.dimension, self.conf.group]), name="w_gc")
        self.b_gc = tf.Variable(self.initializer([1, self.conf.group]), name="b_gc")
    ...
    def constructTrainGraph(self):
        ...
        # 添加子图生成模块
        user_group_emb_hidden1 = tf.nn.leaky_relu(tf.matmul(self.fusion_user_embedding, self.w_gc_1) + self.b_gc_1)
        user_group_emb_hidden1 = tf.nn.dropout(user_group_emb_hidden1, 0.6)
        user_group_emb_sum = tf.matmul(user_group_emb_hidden1, self.w_gc) + self.b_gc

        a_top, a_top_idx = tf.nn.top_k(user_group_emb_sum, 1, sorted=False)
        user_group_embeddings = tf.cast(tf.equal(user_group_emb_sum, a_top), dtype=tf.float32)  # nu, 2

        A_fold_hat_group, A_fold_hat_group_filter = self.getAHatGroup(
            self.social_neighbors_sparse_matrix, user_group_embeddings)

        fusion_user_emb_g = []
        for g in range(0, self.conf.group):
            fusion_user_emb_g.append(self.fusion_user_embedding)

        ego_embeddings_f = []
        all_embeddings = [self.fusion_user_embedding]
        for k in range(0, self.conf.layer):
            for g in range(0, self.conf.group):
                side_emb = tf.sparse_tensor_dense_matmul(A_fold_hat_group[g][0], fusion_user_emb_g[g])
                fusion_user_emb_g[g] = fusion_user_emb_g[g] + side_emb
                if k == 0:
                    ego_embeddings_f.append(tf.sparse_tensor_dense_matmul(self.social_neighbors_sparse_matrix, side_emb))
                else:
                    ego_embeddings_f[g] = tf.sparse_tensor_dense_matmul(self.social_neighbors_sparse_matrix, side_emb)
            fusion_user_emb = tf.reduce_sum(ego_embeddings_f, axis=0)
            all_embeddings += [fusion_user_emb]
        ...
        # self.changeSocialAdj(user_group_embeddings)
        # second_gcn_user_embedding = self.generateUserEmbeddingFromSocialNeighbors(first_gcn_user_embedding)
        # third_gcn_user_embedding = self.generateUserEmbeddingFromSocialNeighbors(second_gcn_user_embedding)
        ...
        self.final_user_embedding = all_embeddings
